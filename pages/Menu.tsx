...
import { getOnlineShopSetting } from '../services/supabase';
...
const Menu: React.FC<MenuProps> = ({ lang }) => {
  ...
  // Aggiorna localStorage quando cambia il carrello
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // Leggi OnlineShop
        const shop = await getOnlineShopSetting();
        setOnlineShop(shop);
      } catch (e: any) {
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {!loading && !error && !menuHeader && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
            ...
            {dishes.map((dish, i) => (
              <div key={dish.id || i} className="flex gap-6 group cursor-pointer bg-white/5 border border-white/10 rounded-xl p-4 md:p-5">
                ...
                <div className="flex-grow pb-2">
                  ...
                  {/* Pulsante Ordina sotto il prezzo */}
                  {onlineShop && (
                    <div className="flex justify-end mt-2">
                      <button
                        className="px-6 py-2 bg-restaurant-accent text-white rounded-lg font-semibold hover:bg-orange-600 transition-all text-lg"
                        onClick={e => {
                          e.stopPropagation();
